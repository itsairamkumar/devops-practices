...
## 6) Commands
    ...
    8. mv : The source file will be moved to the destination folder, and if you provide a new file name, it will be moved with that new name.

        Example : mv /E/DevOps-Notes/01-Linux/Day-08/notes.txt /E/DevOps-Notes/01-Linux/Day-07/linux_notes.txt

    9. rm : Remove (delete) a file or folder

        Example : rm /E/DevOps-Notes/01-Linux/Day-07/linux_notes.txt

    rm -r : `-r` means **recursive**. It is used to remove a folder along with everything inside it, including sub-folders and files.
